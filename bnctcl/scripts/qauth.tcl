...
proc qauth:server {client params} {
	if {[string equal -nocase [lindex $params 0] "Q!TheQBot@CServe.quakenet.org"]} {
		...
		if {[string match -nocase "*CHALLENGE*" [lindex $params 3]]} {
			...
			if {[string match -nocase "*HMAC-MD5*" [lindex $params 3]] && [lsearch $::qauth_supported_algorithms md5] > "-1"} {
			...
			} elseif {[string match -nocase "*HMAC-SHA-256*" [lindex $params 3]] && [lsearch $::qauth_supported_algorithms sha256] > "-1"} {
